<?php
/**
 * Class Mad_Import_Product_CSV_Importer_Controller file.
 */
 ...
 */
class Mad_Import_Product_CSV_Importer_Controller {
  ...
  protected $update_existing = false;
	...
	public static function get_importer( $file, $args = array() ) {
		$importer_class = apply_filters( 'mad_import_product_csv_importer_class', 'Mad_Import_Product_CSV_Importer' );
		$args           = apply_filters( 'mad_import_product_csv_importer_args', $args, $importer_class );
	...
	public static function is_file_valid_csv( $file, $check_path = true ) {
		if ( $check_path && apply_filters( 'mad_import_product_csv_importer_check_import_file_path', true ) && false !== stripos( $file, '://' ) ) {
			return false;
		}
	...
	protected static function get_valid_csv_filetypes() {
		return apply_filters(
			'mad_import_csv_product_import_valid_filetypes', array(
				'csv' => 'text/csv',
				'txt' => 'text/plain',
			)
		);
	...
	public function __construct() {
		...
		$this->steps = apply_filters( 'mad_import_product_csv_importer_steps', $default_steps );
    ...
    $this->type            = isset( $_REQUEST['type'] ) ? wc_clean( wp_unslash( $_REQUEST['type'] ) ) : '';
	}
	...
	public function get_next_step_link( $step = '' ) {
      ...
      'type'       => $this->type,
			'_wpnonce'        => wp_create_nonce( 'mad-import-csv-importer' ), // wp_nonce_url() escapes & to &amp; breaking redirects.,
	...
	public function upload_form_handler() {
		check_admin_referer( 'mad-import-csv-importer' );
	...
	public function handle_upload() {
		...
		if ( empty( $file_url ) ) {
			...
			if ( ! self::is_file_valid_csv( ABSPATH . $file_url ) ) {
				return new WP_Error( 'mad_import_product_csv_importer_upload_file_invalid', __( 'Invalid file type. The importer supports CSV and TXT file formats.', 'ssvmad' ) );
			}
		...
		return new WP_Error( 'mad_import_product_csv_importer_upload_invalid_file', __( 'Please upload or provide the link to a valid CSV file.', 'ssvmad' ) );
	}
	...
	protected function mapping_form() {
		...
		if ( empty( $sample ) ) {
			$this->add_error(
				__( 'The file is empty or using a different encoding than UTF-8, please try again with a new file.', 'ssvmad' ),
				array(
					array(
						'url'   => admin_url( 'tools.php?page=product_importer' ),
						'label' => __( 'Upload a new file', 'ssvmad' ),
					),
				)
			);
	...
	public function import() {
		if ( ! self::is_file_valid_csv( $this->file ) ) {
			$this->add_error( __( 'Invalid file type. The importer supports CSV and TXT file formats.', 'ssvmad' ) );
			$this->output_errors();
			return;
		}

		if ( ! is_file( $this->file ) ) {
			$this->add_error( __( 'The file does not exist, please try again.', 'ssvmad' ) );
			$this->output_errors();
			return;
		}
		...
		if ( ! empty( $_POST['map_from'] ) && ! empty( $_POST['map_to'] ) ) {
			...
			update_user_option( get_current_user_id(), 'mad_import_product_import_mapping', $mapping_to );
		} else {
		...
		wp_localize_script(
			'mad-import-product-import', 'mad_import_product_import_params', array(
				'import_nonce'    => wp_create_nonce( 'mad-import-product-import' ),
				'mapping'         => array(
					'from' => $mapping_from,
					'to'   => $mapping_to,
				),
